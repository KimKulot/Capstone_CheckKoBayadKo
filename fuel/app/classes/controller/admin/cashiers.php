	...
	public function action_add_basic_miscellanous()
	{
		...
		if (Input::method() == 'POST')
		{
			...
			if ($val->run())
			{
				...
				$id_program = Input::post('basic_program_id');
				...
				// var_dump($data['basic_programs']);die;
				foreach ($data['basic_programs'] as $pro) {
	...
	*/
	...
	public function action_view($id = null)
	{
		...
		if($count_exist_basicprogram > 0){
			Response::redirect('admin/cashiers/view_basic/'. $id);
		}
	...
	$x=0;
		...
		for ($i=0; $i < count($arrstatus) ; $i++) { 
			if ($arrstatus[$i] == 'success') {
				Session::set_flash('success', e('Message send successfully'));
			}else{
				Session::set_flash('error', e('Message send failed'));
			}
			 Response::redirect('admin/cashiers');
		}
	...
	public function action_add_miscellanous()
	{
		...
		if (Input::method() == 'POST')
		{
			...
			if ($val->run())
			{
				...
				$id_program = Input::post('program_id');

				$data['student_programs'] = Model_Program::find('all', [
					'where' => [
						['id', 'like', "$id_program"]
					]
				]);

				foreach ($data['student_programs'] as $pro) {
					foreach ($student['students'] as $stud) {
						if($stud->program == $pro->program_description){
							$formula_dis_misc = 0;
