					...
					?>
						<tr>
							...
							<?php if ($boolchecker==false): ?>
								...
								<?php 
								$miscresult = 0;
									foreach ($programs as $college_program) {
										if ($college_program->program_description == $item->program) {
											foreach ($miscellaneous as $checkmisc) {
												$miscresult = $miscresult + $checkmisc->amount;
											}
										}
									}
								
									foreach ($basic_programs as $basic_edu_program) {

										if ($basic_edu_program->basic_program_description == $item->program) {
											foreach ($basic_miscellaneous as $checkmisc) {
												$miscresult = $miscresult + $checkmisc->amount;
											}
										}
								...
								$item->tuition_fee + $miscresult) , 2) ?></td>
								<td><span>&#8369</span><?php echo " " . number_format($item->tuition_fee,2); ?></td>

								
								<td><span>&#8369</span><?php echo " " . number_format($miscresult,2); ?></td>
								...
								<?php  
									$temp_total_balance =  $miscresult - $item->dis_misc;

									?>
								<!-- end calculation for total balance -->
								<td><span>&#8369</span><?php echo " " . number_format($item->balance + abs($temp_total_balance),2);?></td>
